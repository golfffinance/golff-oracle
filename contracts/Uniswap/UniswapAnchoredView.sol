...
contract UniswapAnchoredView is UniswapConfig {
    ...
    function setReporter(address _reporter) public {
    ...
    function postPrices2(address[] calldata sources, uint64[] calldata timestamps, string[] calldata keys, uint64[] calldata values, string[] calldata symbols) external {
        require(msg.sender == reporter, "!reporter ");
        require(sources.length == timestamps.length && timestamps.length == keys.length && keys.length == values.length, "Parameter array must be 1:1");

        // Save the prices
        for (uint i = 0; i < sources.length; i++) {
            priceData.put2(sources[i], timestamps[i], keys[i], values[i]);
        }
